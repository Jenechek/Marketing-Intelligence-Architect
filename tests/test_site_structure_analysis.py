...
@pytest.mark.parametrize(
    ("outlier_depth", "expected"),
    ((4, False), (5, True)),
)
def test_anomalous_depth_requires_minimum_depth_five_above_p90(
    outlier_depth: int, expected: bool
):
    base = f"https://minimum-depth-{outlier_depth}.test/"
    pages = []
    for index in range(1, 11):
        links = (base + str(index + 1),) if index < 10 else ()
        depth = outlier_depth if index == 10 else (0 if index == 1 else 1)
        pages.append(raw(index, index, base + str(index), depth=depth, links=links))
    structure = build_site_structure(tuple(pages))
    page = structure.analysis.page_by_id(10)
    assert page is not None
    assert page.has_signal(StructuralSignal.ANOMALOUS_DEPTH) is expected
